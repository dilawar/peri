...
def name_globals(s, remove_params=None):
    ...
    """
    all_params = s.params
    for p in s.param_particle(list(range(s.obj_get_positions().shape[0]))):
        all_params.remove(p)
...
class LMParticles(LMEngine):
    ...
    * ``_MINDIST`` : 1e-3
    ...
    def __init__(self, state, particles, include_rad=True, **kwargs):
        ...
        self._is_rad = np.array([x in rad_nms for x in self.param_names])
        pos_nms = self.state.param_positions()
        self._is_pos = []
        for a in range(3):  # FIXME explicit 3D
            self._is_pos.append(np.array([(x in pos_nms) &
                    (x[-1] == 'zyx'[a]) for x in self.param_names]))
...
class AugmentedState(object):
    ...
    def __init__(self, state, param_names, rz_order=3):
        ...
        has_rad = [x in param_names for x in rad_nms]
        if np.any(has_rad):
